...
public class Tokenize {
	public static void  main(String [] args) throws FileNotFoundException{
		...
		String text = "\tSussex result matter any end see. It speedily me addition weddings vicinity";
		        
		int start = 0; int end = text.length();
				LinkedList<Integer> tokens = tokenize(text, 0, end);

        for(int stop : tokens){

            System.out.println("word:" + text.subSequence(start, stop).toString() +
                    "|start:" + start +
                    "|stop:" + stop +
                    "|length:" + text.length());
