...
public class DocumentLayout {
    ...
    class TextSize {
        ...
        public TextSize(float width, float height){
            this.width = width;
            this.height = height;
        }
    }

    public TextSize measureText(CharSequence sequence, Paint paint){
        StaticLayout staticLayout = new StaticLayout(sequence, (TextPaint) paint,
                (int) params.parentWidth, android.text.Layout.Alignment.ALIGN_NORMAL, 1, 0, false);
        return new TextSize(staticLayout.getLineWidth(0), staticLayout.getHeight());
    }

    public TextSize measureText(String sequence, Paint paint){
        return new TextSize(paint.measureText(sequence), getFontAscent() + getFontDescent());
    }
    ...
    public void measure() {
        ...
        float x, y = lineHeight + params.top, spaceOffset = measureText(" ", paint).width;
        ...
        for (String paragraph : chunks) {
            ...
            while (true) {
                ...
                for (int i = format.start; i < format.end; i++) {
                    ...
                    x += offset + measureText(unit.unit, paint).width + spaceOffset;
