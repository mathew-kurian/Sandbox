...
app.tools = {
	...
	queryToUrl :  function(q, cb) {
	           
	    var ytbRegex = /youtu[.]{0,1}be([.]{0,1}com)?\/(.*)/g,
	        ytbLink = "http://www.youtube.com/watch?v=",
	        ytbMatched = q.match(ytbRegex);
	   
	    if(ytbMatched == null || typeof ytbMatched === 'undefined' || ytbMatched.length == 0){
	      
	    	console.log("[YoutubeFeeds]Query = " + q); 

	        app.feeds.videos( {q: q, 'max-results':  10, orderby : 'relevance' }, function(err, data){
	            ...
	            if(typeof results === 'undefined' || results.length == 0){
	                return cb("No matching videos.");
	            } 
	            
	            return cb(0, ytbLink + data.items[0].id, data);
	        }); 

	    } else {
	    	
	        // Already a valid url
	        cb(0, "https://www." + ytbMatched[0]);
	    }
	}
}
