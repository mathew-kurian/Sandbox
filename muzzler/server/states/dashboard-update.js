...
require('elixir/cstate').extend({
    ...
    initNotifications : function(){
        ...
        var handleProcessFailure = function(data, dom, id, ecode, eid){
        ...
        handler.on('process', 'failure', toDOM(handleProcessFailure));
    ...
    ready: function() {
        ...
        toolkit.bind('.dashboard-sidebar-btn', '#dashboard-sidebar', listeners, 'click', function(e) {
            var $id = $(this).attr('id');
            var state = $id.substring($id.lastIndexOf('-') + 1) + "-update";
        ...
        var ended = function(audio, sid, pid) {
            if (!_.private().queueEnabled && repeatOnce) return handler.safePlay(sid, pid);
        ...
        // toolkit.bind('#music-next', 'body', listeners, 'click', function(e) {
        //     if(_.private().queueEnabled) {
