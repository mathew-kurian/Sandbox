    ...
    function Emitter(attachTo, invokeAfter){
        ...
        self.fireListeners = function(type){
            ...
            if(!silent && ignore.indexOf(type) < 0) {
                console.t('emitter.js').info('Listeners fired for on `%s` event.'.format(type));
            }
        }
        ...
        self.on = function(type, callback){
            ...
            if(!silent) console.t('emitter.js').warn('Listener add for on `%s` event.'.format(type));
            return self;
        }
        ...
        self.off = function(type, callback){
            ...
            try { 
                ...
                if(!silent) console.t('emitter.js').info('Listener removed for on `%s` event.'.format(type));
            } catch(e){
                if(!silent) console.t('emitter.js').info('Listener could not be removed for on `%s` event.'.format(type));
            } 
