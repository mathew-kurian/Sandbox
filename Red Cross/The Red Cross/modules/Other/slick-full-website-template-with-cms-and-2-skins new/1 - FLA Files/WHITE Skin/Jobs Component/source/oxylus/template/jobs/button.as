...
class oxylus.template.jobs.button extends MovieClip 
{
	...
	public function setNode(n:XMLNode) {
		...
		read["title"]["txt"].autoSize = true;
		read["title"]["txt"].text = node.attributes.readMoreTitle;
		
		apply["title"]["txt"].autoSize = true;
		apply["title"]["txt"].text = node.attributes.uploadTitle;		
		apply._x = Math.round(read._x + read._width + 12 );
