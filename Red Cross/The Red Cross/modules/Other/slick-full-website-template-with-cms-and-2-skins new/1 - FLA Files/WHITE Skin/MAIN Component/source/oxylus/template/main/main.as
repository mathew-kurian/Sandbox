...
// uncomment and use this line then comment the following if you are testing online and want to make sure the xml file is not cached					
//					xmlOb.load(currentXml +"?nocache="+ String((new Date()).getTime()));
	...
	private function resize(w:Number, h:Number) {
		...
		graphics["bottom"]._x = Math.round(w / 2 - graphics["bottom"]._width / 2);
		
		graphics["top"]._x = Math.round(w/2 - graphics["top"]._width/2);
