...
class COXYMallJobs extends CPlugin{
	...
	function SendResume() {
		...
		if ($_POST["file"] != "nofile") {
			...
			$vars["attachment_link"] = $_CONF["url"]. "resumes/" . $resume["resume_code"];
		} else 
		...
		if ($this->tpl_module["settings"]["set_email_1"]) {
			...
			$email = array(
				...
				"email_from" => $this->tpl_module["settings"]["set_email_1_from"],
		...
		if ($this->tpl_module["settings"]["set_email_2"]) {
				...
				"email_from" => $this->tpl_module["settings"]["set_email_2_from_email"],
				"email_from_name" => $this->tpl_module["settings"]["set_email_2_from_name"],
