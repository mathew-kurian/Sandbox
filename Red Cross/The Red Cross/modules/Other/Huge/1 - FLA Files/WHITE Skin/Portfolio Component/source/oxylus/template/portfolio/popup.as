...
class oxylus.template.portfolio.popup extends MovieClip
{
	...
	private function loadPicture() {
		...
		panObj.src = str;
		panObj.pan = node.firstChild.firstChild.attributes.pan;
		panObj.maxWidth = Number(node.firstChild.firstChild.attributes.maxWidth);
		panObj.maxHeight = Number(node.firstChild.firstChild.attributes.maxHeight);
		...
		if ((arr[arr.length - 1] == "flv") || (arr[arr.length - 1] == "mov") || (arr[arr.length - 1] == "mp4") || (arr[arr.length - 1] == "h264")) {
			...
			if (panObj.pan == 1) {
				loaderRemove();
				img._alpha = 100;
				img.createEmptyMovieClip("pan", img.getNextHighestDepth());
				img["pan"].attachMovie("IDOxylusAdvancedPanningControls", "thePan", img["pan"].getNextHighestDepth());
				
				var myPanObj:Object = new Object();
				myPanObj.objectPath = panObj.src;
				myPanObj.viewportWidth = panObj.maxWidth;
				myPanObj.viewportHeight = panObj.maxHeight;
				
				myPanObj.minimumZoom ="auto"
				myPanObj.maximumZoom =3
				myPanObj.initialZoom =1
				myPanObj.initPosXPercent =.5
				myPanObj.initPosYPercent =.5
				myPanObj.mouseWheelZoom =true
				myPanObj.controllerType = 1
				myPanObj.smoothImage = true
				myPanObj.zoomStepPercent = .5
				
				myPanObj.showController = true
				myPanObj.ctrlXAlign = "left"
				myPanObj.ctrlYAlign = "bottom"
				myPanObj.maxSlidingStep =10
				myPanObj.edgeTolerance =10
				myPanObj.panningType =0
				myPanObj.nudgeStep =20
				
				img["pan"]["thePan"].onConfigData(myPanObj);
	...
	private function buildBackground() {
		drawOval(bg, tw, th, 6, 0xffffff, 90);
	}
	...
	private function buildNavigation() {
		drawOval(navigation["bg"], tw + 80, 196, 6, 0xffffff, 80);
	...
	if (panObj.pan != 1) {
		...
		if (init == 1) {
					tw = panObj.maxWidth + 60;
					th = panObj.maxHeight + 80 + 140;
	...
	private function updateOthersByHtml() {
			...
			drawOval(bg, tww, thh, 6, 0xffffff, 90);
			 
			drawOval(navigation["bg"], tww + 80, 196, 6, 0xffffff, 80);
			next._x = Math.round(navigation["bg"]._width - next._width - 8);
			prev._x = 8;
			next._y = prev._y = Math.round(navigation["bg"]._height / 2 - next._height / 2);
			//addShadow(navigation["bg"]);
	...
	private function drawOval(mc:MovieClip, mw:Number, mh:Number, r:Number, fillColor:Number, alphaAmount:Number) {
		...
		mc.lineStyle(1, 0xc3c3c3, alphaAmount, true);
		mc.moveTo(r,0);
		mc.lineTo(mw-r,0);
