...
class COXYMallServices extends CPlugin{
		...
		if (strstr($_GET["sub"] , "oxymall.plugin.services.")) {
			...
			switch ($sub) {
				...
				case "landing":
					...
					$data = new CSQLAdmin("services/" . $sub, $this->__parent_templates,$this->db,$this->tables,$extra);
	...
	*/
	function PrepareFields($forms , $sub) {
		...
		$forms["add"]["fields"]["item_image"]["thumbnails"]["resize"]["width"] = $this->tpl_module["settings"]["set_imagewidth"];

		$forms["edit"]["fields"]["item_image"]["thumbnails"]["resize"]["height"] = 
		$forms["add"]["fields"]["item_image"]["thumbnails"]["resize"]["height"] = $this->tpl_module["settings"]["set_imageheight"];
		...
		$forms["edit"]["fields"]["item_image"]["title"] = CTemplateStatic::ReplaceSingle(
			...
			"size" , 
			$this->tpl_module["settings"]["set_imagewidth"] . " x "  . $this->tpl_module["settings"]["set_imageheight"]
		);
	...
	*/
	function StoreCat($record) {
		if ($record["cat_id"] && !$record["cat_order"]) {
			$this->db->QueryUpdate(
				$this->tables["plugin:services_cats"],
				array(
	...
	function __call_reorder_cats($action) {
		...
		$sql = "SELECT * FROM {$this->tables['plugin:services_cats']} WHERE module_id={$this->tpl_module[mod_id]} AND cat_order " . ($_GET["action"] == "moveup" ? " <= " : " >= " ) . " '{$_GET[cat_order]}' ORDER BY cat_order " . ($_GET["action"] == "moveup" ? " DESC " : " ASC " ) . " LIMIT 2 " ;
	...
	function __call_reorder_images($action) {
		...
		$item = $this->db->QFetchArray("SELECT * FROM {$this->tables['plugin:services_items']} WHERE item_id='{$_GET[item_id]}' ");


		if ($this->tpl_module["settings"]["set_reverseorder"]) {
			$sql = "SELECT * FROM {$this->tables['plugin:services_items']} WHERE module_id={$this->tpl_module[mod_id]} AND item_cat='{$item[item_cat]}' AND item_order " . ($_GET["action"] == "moveup" ? " >= " : " <= " ) . " '{$_GET[item_order]}' ORDER BY item_order " . ($_GET["action"] == "moveup" ? " ASC " : " DESC " ) . " LIMIT 2 " ;
		} else {
			$sql = "SELECT * FROM {$this->tables['plugin:services_items']} WHERE module_id={$this->tpl_module[mod_id]} AND item_cat='{$item[item_cat]}' AND item_order " . ($_GET["action"] == "moveup" ? " <= " : " >= " ) . " '{$_GET[item_order]}' ORDER BY item_order " . ($_GET["action"] == "moveup" ? " DESC " : " ASC " ) . " LIMIT 2 " ;
		}
