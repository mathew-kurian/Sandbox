...
class COXYMallModules extends CPlugin{
		...
		if (strstr($_GET["sub"] , "oxymall.plugin.modules.")) {
			...
			switch ($sub) {
				...
				case "user":
					...
					if (!in_array($_GET["action"] , array("edit" , "details" , "add" , "store" , "delete"))) {
						$data = new CSQLAdmin("modules/" . $sub, $this->__parent_templates,$this->db,$this->tables,$extra);
						$modules = $data->form->SimpleList($data->forms["forms"]["list"] , "" , "" , null , false, true);
						$modules = CFORM::__private_tree($modules["items"] , 0 , "mod_id" , "mod_parent" , "<img width=20 height=0 border=0/>" , 0);
		
						if (is_array($modules)) {
							foreach ($modules as $key => $val) {
								$modules[$key]["mod_name"] = $val["separator"] . $val["mod_name"];
							}						
						}					
						return $data->FormList($modules);
	...
	*/
	...
	function __call_reorder_user_modules($action) {
		...
		$sql = "SELECT * FROM {$this->tables['core:user_modules']} WHERE mod_parent={$_GET[mod_parent]} AND mod_order " . ($_GET["action"] == "moveup" ? " <= " : " >= " ) . " '{$_GET[mod_order]}' ORDER BY mod_order " . ($_GET["action"] == "moveup" ? " DESC " : " ASC " ) . " LIMIT 2 " ;
