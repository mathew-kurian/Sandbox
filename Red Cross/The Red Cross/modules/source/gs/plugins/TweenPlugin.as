...
class gs.plugins.TweenPlugin {
		...
		private function addTween($object:Object, $propName:String, $start:Number, $end:Object, $overwriteProp:String):Void {
			if ($end != undefined) {
				...
				if (change != 0) { //don't tween values that aren't changing! It's a waste of CPU cycles
					_tweens[_tweens.length] = new TweenInfo($object, $propName, $start, change, $overwriteProp || $propName);
				}
			}
		}
		...
		private function updateTweens($changeFactor:Number):Void {
			...
			if (this.round) {
				for (i = _tweens.length - 1; i > -1; i--) {
					ti = _tweens[i];
					ti.target[ti.property] = Math.round(ti.start + (ti.change * $changeFactor));
				}
		...
		public function killProps($lookup:Object):Void {
			...
			for (i = this.overwriteProps.length - 1; i > -1; i--) {
				if ($lookup[this.overwriteProps[i]] != undefined) {
					this.overwriteProps.splice(i, 1);
				}
			}
			for (i = _tweens.length - 1; i > -1; i--) {
				if ($lookup[_tweens[i].name] != undefined) {
					_tweens.splice(i, 1);
				}
			}
		}
