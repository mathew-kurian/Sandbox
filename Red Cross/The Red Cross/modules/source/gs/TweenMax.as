...
class gs.TweenMax extends TweenLite {
	...
	public function setDestination($property:String, $value:Object, $adjustStartValues:Boolean):Void {
		var p:Number = this.progress, i:Number;
		if (this.initted) {
			if ($adjustStartValues == false && p != 0) {
				for (i = this.tweens.length - 1; i > -1; i--) {
					if (this.tweens[i].name == $property) {
						this.tweens[i].target[this.tweens[i].property] = this.tweens[i].start; //return it to its start value
					}
				}
			}
			...
			var v:Object = {}, j:Number, overwriteProps:Array;
